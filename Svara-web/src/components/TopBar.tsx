  ...
  return (
    <div className="relative flex items-center gap-2 md:gap-3 group" ref={menuRef}>
      ...
      <Link href="/" className="hidden lg:block">
        <span className="text-white font-black text-xl tracking-tight hover:text-primary transition-colors cursor-pointer">{siteConfig.appName}</span>
      </Link>
      ...
      {isOpen && (
        <div className="absolute top-[calc(100%+12px)] left-0 w-48 md:w-56 bg-black/90 md:bg-[#282828] rounded-md shadow-[0_16px_40px_rgba(0,0,0,0.8)] border border-white/[0.05] py-1 z-[4000] animate-in fade-in zoom-in duration-150">
            <div className="px-4 py-3 border-b border-white/[0.05] mb-1">
              <p className="text-white text-[15px] font-black tracking-tight truncate">Nishant</p>
              <p className="text-text-subdued text-[10px] font-black uppercase tracking-[0.15em] mt-0.5">Builder of Svara</p>
            </div>
            
            <ProfileLink href={siteConfig.androidReleaseUrl} label="Download Android App" external />
            <ProfileLink href={siteConfig.githubUrl} label="GitHub Repository" external />
            <ProfileLink href={siteConfig.portfolioUrl} label="Portfolio" external />
            <ProfileLink href="/jam" label="Jam Connect" />
            ...
            <div className="px-4 pb-2 opacity-30 text-center">
               <span className="text-[9px] font-black uppercase tracking-widest text-white">{siteConfig.appName} &copy; 2026</span>
            </div>
        </div>
      )}
    </div>
...
const ProfileLink = ({ href, label, external }: { href: string; label: string; external?: boolean }) => {
  const className =
    "flex items-center justify-between px-4 py-3 text-white/90 hover:text-white hover:bg-white/[0.1] transition-colors group";
  ...
  return (
    <div className="flex-1 flex items-center justify-between gap-3 md:gap-4 h-full pointer-events-auto">
      ...
      <div className={`flex items-center gap-2 md:gap-4 shrink-0 ${isSearchPage ? 'hidden md:flex' : 'flex'}`}>
        ...
        <Link
          href="/jam"
          className="hidden sm:flex items-center gap-2 bg-white/8 text-white px-4 py-2 rounded-full text-[14px] font-bold transition-all hover:bg-white/12 active:scale-95"
        >
