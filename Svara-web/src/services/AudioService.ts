...
export class AudioService {
  ...
  private autoplayWarningShown = false;
        ...
        onplayerror: (id, err) => {
          if (this.isAutoplayRestrictionError(err)) {
            if (!this.autoplayWarningShown) {
              console.warn('[AudioService] Playback blocked until a user interaction unlocks audio.');
              this.autoplayWarningShown = true;
            }
            this.emit('pause', null);
            this.emit('loading', false);
            this.howl?.once('unlock', () => {
              this.autoplayWarningShown = false;
              this.howl?.play();
            });
            return;
          }
  ...
  private isAutoplayRestrictionError(err: unknown) {
